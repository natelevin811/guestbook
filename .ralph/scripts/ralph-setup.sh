#!/bin/bash
# Ralph Wiggum: Interactive Setup & Loop (Claude Code Edition)
#
...
# Global workspace for cleanup access
GLOBAL_WORKSPACE=""
...
cleanup() {
  ...
  if [[ -n "$GLOBAL_WORKSPACE" ]] && [[ -f "$GLOBAL_WORKSPACE/.ralph/state/activity.log" ]]; then
    echo "🧹 Clearing activity log..."
    echo "# Activity Log (cleared on shutdown)" > "$GLOBAL_WORKSPACE/.ralph/state/activity.log"
  fi
...
# Model options (Claude Code aliases and full names)
MODELS=(
  "opus"
  "sonnet"
...
main() {
  ...
  # Walk the task chain to find current incomplete task
  ...
  if [[ -z "$task_file" ]]; then
    echo ""
    echo "🎉 All tasks in chain are complete! Nothing to do."
    exit 0
  fi
  ...
  export USE_BRANCH
  export OPEN_PR
  export CURRENT_TASK_FILE="$task_file"

  # Handle single iteration first
  if [[ "$run_single_first" == "true" ]]; then
    echo ""
    echo "🧪 Running single iteration first on: $task_file_rel"
    echo ""
    ...
    while [[ $iteration -le $MAX_ITERATIONS ]]; do
      ...
      case "$signal" in
        ...
        "GUTTER")
          ...
          echo "🚨 Gutter detected. Check .ralph/state/errors.log"
          exit 1
          ;;
        *)
